...
import { History, Search, Filter, Copy, Download, ExternalLink, Trash2, Archive, Star, Clock, Globe, Wifi, FileText, Sparkles } from 'lucide-react';
...
const HistoryPanel: React.FC<HistoryPanelProps> = ({ recentQRCodes, onClearHistory }) => {
  ...
  useEffect(() => {
    ...
    chrome.storage.local.get(['qrHistory'], (result) => {
      if (result.qrHistory) {
        setAllQRCodes(result.qrHistory);
      }
    });
  }, [recentQRCodes]);
